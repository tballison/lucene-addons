...
public class RequestThreads<P> extends ArrayList<RequestWorker> implements ExecutorService
{
	...
	public P getMetadata() { return metadata;}
	public RequestThreads<P> setMetadata(P value) {metadata = value; return this;}
	...
	@Override public void execute(Runnable command) { exe.execute(command); }
	@Override public boolean awaitTermination(long timeout, TimeUnit unit) throws InterruptedException { return exe.awaitTermination(timeout, unit); }
	@Override public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks) throws InterruptedException { return exe.invokeAll(tasks); }
	...
	@Override public <T> T invokeAny(Collection<? extends Callable<T>> tasks) throws InterruptedException, ExecutionException { return exe.invokeAny(tasks); } 	
	...
	@Override public boolean isShutdown() { return exe.isShutdown(); }
	@Override public boolean isTerminated() { return exe.isTerminated(); }
	@Override public void shutdown() { exe.shutdown(); 	}
	@Override public List<Runnable> shutdownNow() { return exe.shutdownNow(); }
	@Override public <T> Future<T> submit(Callable<T> task) { return exe.submit(task); }
	@Override public Future<?> submit(Runnable task) { return exe.submit(task); }
	@Override public <T> Future<T> submit(Runnable task, T result) { return exe.submit(task, result); }
	public void seal() { exe.shutdown(); 	}
